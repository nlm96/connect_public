...
class CreateSingleDataFile():
    ...
    def join(self):
        ...
        if self.param.use_likelihood_filter and self.param.sampling == 'iterative':
            i = 0
            with open(os.path.join(self.path, 'likelihood_data.txt'), 'w') as f:
                for filename in sorted(os.listdir(os.path.join(self.path, 'likelihood_data'))):
                    if filename.endswith('.txt'):
                        with open(os.path.join(self.path, 'likelihood_data', filename), 'r') as g:
                            for line in g:
                                if line[0] == '#' and i == 0:  # Write header only once
                                    f.write(line)
                                    i = 1
                                elif line[0] != '#':  # Write data lines
                                    f.write(line)
