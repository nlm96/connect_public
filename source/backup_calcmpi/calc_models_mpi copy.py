...
if param.memory_safe and rank > 0:
    ...
    def get_soft_limit():
        ...
        soft_limit_bytes = int(0.8 * available_bytes)
    ...
    def respawn_child():
        ...
        def timeout_handler(signum, frame):
            """Handles timeout if a worker becomes unresponsive."""
            raise TimeoutError("Worker rank unresponsive!")
...
if rank == 0:
    ...
    TIMEOUT_SECONDS = 240  # Timeout for rank response
    ...
    while len(data) > data_idx:
        ...
        try:
            # Start timeout before waiting for response
            ...
            print(
                f"[Master] WARNING: Rank {r} is unresponsive. Marking as failed.",
                flush=True,
            )
            failed_ranks.add(r)  # Add to failed ranks so it's no longer assigned work
        except MPI.Exception as e:
            print(f"[Master] ERROR: MPI error with rank {r}: {repr(e)}", flush=True)
            failed_ranks.add(r)
        finally:
            # Reset alarm after each iteration
            signal.alarm(0)
...
## rank > 0 (slaves)
    ...
    while True:
        ...
        if param.use_likelihood_filter and param.sampling == "iterative":
            ...
            if type(received_data).__name__ == "str":
                print(
                    f"{prefix} Received 'Done' message from master. exiting now.",
                    flush=True,
                )
                break
            ...
            if param.memory_safe:
                ...
                while True:
                    ...
                    try:
                        ...
                        child_comm.send("PING", dest=0, tag=99)

                        while not child_comm.iprobe(
                            source=0, tag=99
                        ):  # Use iprobe before irecv
                            time.sleep(0.1)

                        # Message has arrived, now receive it (non-blocking receive)
                        command = child_comm.recv(source=0, tag=99)
                        ...
                        if command != "PONG":
                            ...
                            # Retry once before considering a respawn
                            while not child_comm.iprobe(source=0, tag=99):
                                time.sleep(0.1)
                    ...
                    except (MPI.Exception, TimeoutError) as e:
                        print(
                    ...
                    if (
                        ...
                        try:
                            ...
                            sleep_initial = (
                                0.001  # 0.0001  # Start with a reasonable value
                            )
                            sleep_max = 0.1  # 0.1  # Avoid wasting CPU if the child takes a long time
                            sleep_current = sleep_initial  # Start at 0.01s

                            while not child_comm.iprobe(source=0, tag=0):
                                time.sleep(sleep_current)  # Wait before checking again
                                sleep_current = min(sleep_current * 2, sleep_max)
    ...
    if param.memory_safe:
        ...
        print(
            f"{prefix} [DEBUG] Sending 'DONE' message to child process...", flush=True
        )
        ...
        try:
            ...
            child_comm.send(("DONE", command_data), dest=0, tag=0)
            ...
            if child_comm.iprobe(source=0, tag=0):
...
# Timeout handler: attempt clean exit first, then force exit if necessary
def timeout_handler2(sig, frame):
    print(
        f"{prefix} WARNING: MPI_Finalize() timed out. Attempting sys.exit(0)...",
        file=sys.stderr,
        flush=True,
    )

    try:
        sys.exit(0)  # Attempt a normal exit
    except SystemExit:
        pass  # If sys.exit(0) works, nothing else is needed
    except Exception as e:
        print(
            f"{prefix} ERROR: sys.exit(0) failed. Reason: {repr(e)}",
            file=sys.stderr,
            flush=True,
        )

    print(
        f"{prefix} WARNING: Forcing process exit with os._exit(0)",
        file=sys.stderr,
        flush=True,
    )
    os._exit(0)  # Hard exit if sys.exit(0) doesn't work


# Register the timeout handler
signal.signal(signal.SIGALRM, timeout_handler2)
signal.alarm(30)  # Set a timeout for 30 seconds


try:
    print(f"{prefix} calling MPI_Finalize()...", file=sys.stderr, flush=True)
    MPI.Finalize()
    print(f"{prefix} successfully finalized MPI.", file=sys.stderr, flush=True)
except Exception as e:
    print(
        f"{prefix} WARNING: Failed to finalize MPI. Error: {repr(e)}",
        file=sys.stderr,
        flush=True,
    )
finally:
...
print(f"{prefix} Rank {rank} finished.", file=sys.stderr, flush=True)
sys.exit(0)
