...
class Sampling:
    ...
    def create_iterative_data(self):
        ...
        while True:
            ...
            if self.param.use_likelihood_filter:
                if self.param.keep_first_iteration:
                    if i > int(not self.param.keep_initial_data) and i <= i_converged:
                        ...
                        print(
                            f"Accepted {N_accepted} points out of {N_keep}", flush=True
                        )
                        if i > 1 or self.param.keep_initial_data:
                            N_in_data_set = (
                                mcmc.get_number_of_data_points(i - 1) + N_accepted
                            )
            ...
            print(
                f"Calculating {N_accepted} CLASS models ({datetime.datetime.now})",
                flush=True,
            )
            ...
            if kill_iteration and N_accepted < 0.1 * N_in_data_set and not annealing:
                ...
                print(
                    f"Calculations completed at time: {datetime.datetime.now()}",
                    flush=True,
                )
                break
    ...
    def call_calc_models(self, sampling="lhc"):
                ...
                if result.stdout.strip():
                    ...
                    for node, pids in pids_by_node.items():
                        ...
                        if kill_result.returncode != 0 or kill_result.stderr.strip():
                            ...
                            print(
                                f"Kill command successful.", flush=True, file=sys.stderr
                            )
                    ...
                    print(
                        "No lingering MPI processes found.", flush=True, file=sys.stderr
                    )
        ...
        def check_calc_completed():
            ...
            def get_number_of_samples(iter_directory):
                total_samples = 0
                folder_path = os.path.join(iter_directory, "model_params_data")

                # Filter files directly without sorting or extracting file IDs
                file_list = [
                    f
            ...
            if total_samples < 0.8 * target_samples:
                print(
                    f"ERROR / WARNING: Only {total_samples} samples were generated, less than 80% of the target {target_samples}.\n"
        ...
        try:
            ...
            sp.check_call(
                f"mpirun --mca orte_abort_on_non_zero_status 0 --mca orte_allowed_exit_without_sync 1 -np {self.N_tasks - 1} python {self.CONNECT_PATH}/source/calc_models_mpi.py {self.param.param_file} {self.CONNECT_PATH} {sampling} {unique_id}".split(),
            )

            if not check_calc_completed():
                print(
    ...
    def train_neural_network(self, sampling="lhc", output_file=None):
        ...
        if self.param.use_likelihood_filter and self.param.sampling == "iterative":
            ...
            if not self.param.keep_initial_data and iter_num == None:
                print(
                ...
                Likelihood_Filter = LikelihoodFilter(
                    param=self.param, iter_num=iter_num, CONNECT_PATH=self.CONNECT_PATH
                )
