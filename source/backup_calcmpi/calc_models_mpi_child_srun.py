...
print(
    f"{prefix} [DEBUG] Starting child process with ID {child_id}",
    file=sys.stderr,
    flush=True,
)
...
print(f"{prefix} [DEBUG] Importing modules", flush=True, file=sys.stderr)

from source.default_module import Parameters

from source.tools import get_computed_cls, get_z_idx, get_covmat

print(f"{prefix} [DEBUG] Modules imported", flush=True, file=sys.stderr)

print(f"{prefix} [DEBUG] Loading parameters", flush=True, file=sys.stderr)

param_file = os.path.join(CONNECT_PATH, param_file)
param = Parameters(param_file)
param_names = list(param.parameters.keys())

print(f"{prefix} [DEBUG] Parameters loaded", flush=True, file=sys.stderr)
...
print(f"{prefix} [DEBUG] Loading likelihood filter", flush=True, file=sys.stderr)
if (
...
print(f"{prefix} [DEBUG] Likelihood filter loaded", flush=True, file=sys.stderr)
...
print(f"{prefix} [DEBUG] Setting up signal handlers", flush=True, file=sys.stderr)

import signal

cosmo = None

print(f"{prefix} [DEBUG] Setting up handle_signal", flush=True, file=sys.stderr)
...
print(f"{prefix} [DEBUG] Setting up many signal handlers", flush=True, file=sys.stderr)
...
print(f"{prefix} [DEBUG] Setting up timeout handler", flush=True, file=sys.stderr)

signal.signal(signal.SIGALRM, timeout_handler)

print(f"{prefix} [DEBUG] Timeout handler set up", flush=True, file=sys.stderr)

print(f"{prefix} [DEBUG] Starting main_child", flush=True, file=sys.stderr)
...
def main_child():
    ...
    print(f"{prefix} [DEBUG] main child loop started", flush=True, file=sys.stderr)
        ...
        while not stop_memory_monitor.is_set():  # Exit if stop event is set
            ...
            if mem_usage > current_limit:
                ...
                try:
                    ...
                    write_msg(
                        "MEMORY_EXCEEDED", sys.stdout.buffer, {"placeholder": "None"}
                    )
                except Exception as e:
                    print(
                        f"{prefix} ERROR: Failed to notify parent about memory limit: {repr(e)}",
                        file=sys.stderr,
                        flush=True,
                    )
                    print(
    ...
    print(
        f"{prefix} [DEBUG] sys.executable: {sys.executable}",
        flush=True,
        file=sys.stderr,
    )
    print(
        f"{prefix} [DEBUG] instantiating likelihood calculator",
        flush=True,
        file=sys.stderr,
    )
    try:
        if param.use_likelihood_filter and param.sampling == "iterative":
            ...
            write_msg("COSMO_ARGS", sys.stdout.buffer, initial_cosmo_arguments)
    ...
    print(
        f"{prefix} [DEBUG] likelihood calculator instantiated successfully",
        flush=True,
        file=sys.stderr,
    )
    ...
    sample_number = 0
    time_computation = True  # Whether to measure time of computation
    time_cosmo = None
    time_lkl = None
    if time_computation:
    ...
    while True:
        ...
        try:
            print(
                f"{prefix} [DEBUG] Waiting for command from parent",
                file=sys.stderr,
                flush=True,
            )
            command, command_data = read_msg(sys.stdin.buffer)
        ...
        if command == "DONE":
            ...
            try:
                write_msg("DONE_ACK", sys.stdout.buffer)
                ...
                print(
                    f"{prefix} ERROR: Failed to acknowledge shutdown: {repr(e)}",
                    file=sys.stderr,
                    flush=True,
                )
                print(
        ...
        try:
            ...
            if success_cosmo:
                ...
                write_msg("MODEL_DONE", sys.stdout.buffer, result)
            else:
                write_msg("MODEL_FAILED", sys.stdout.buffer, result)
        except Exception as e:
            print(
            ...
            print(
                f"This happened after the CLASS computation for model: {model}",
                file=sys.stderr,
                flush=True,
            )
        ...
        if (
            ...
            try:
                ...
                if success_lkl:
                    write_msg("LIKELIHOOD_DONE", sys.stdout.buffer, result)
                else:
                    write_msg("LIKELIHOOD_FAILED", sys.stdout.buffer, result)
            except Exception as e:
                print(
                    f"{prefix} ERROR: Failed to send message to parent: {repr(e)}",
                    file=sys.stderr,
    ...
    def write_msg(command, output_stream, data=None):
        ...
        Example:
            - write_msg("PING", output_stream)  # Sends "PING" without data
            - write_msg("UPDATE", output_stream, some_object)  # Sends "UPDATE FILE_PATH"
        """
        if data is None:
    ...
    def read_msg(input_stream):
        ...
        Example:
            - read_msg(input_stream)  # ("PING", None) if only a command was sent
            - read_msg(input_stream)  # ("UPDATE", some_object) if data was sent
        """
