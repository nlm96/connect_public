...
class LikelihoodCalculator(ABC):
    ...
    def loglkl(self, prop, cosmo):
        ...
        try:
            ...
            raise ValueError(
                "Severe exception occurred in likelihood computation. Stopping process."
            )
...
def create_likelihood_calc_input_files(param):
    ...
    if param.mcmc_sampler == "montepython":
        likelihood_calc_param_file = os.path.join(
            path_lkl_calc, "likelihood_calc.param"
        )
        param_template_file = (
            "mcmc_plugin/mp_param_templates/connect_lite.param.template"
        )
            ...
            with open(likelihood_calc_param_file, "w") as output:
                ...
                for line in template:
                    ...
                    # Add cosmological parameters
                        ...
                        for custom_par, cdict in param.custom_parameters.items():

                            all_params[cdict["maps_to"]] = cdict[
                                "range"
                        ...
                        for par, interval in all_params.items():
                            ...
                            guess = param.bestfit_guesses.get(
                                par, (interval[0] + interval[1]) / 2
                            )
                            sig = param.sigma_guesses.get(
                                par, abs((interval[1] - interval[0]) / 50)
                            )
                            ...
                            """
                            Normally, if 'par' is in param.log_priors, CONNECT:
                                1) transforms xmin/xmax/guess/sig to log10 space
                                2) appends "_log10_prior" to the parameter name
                            
                            Because we are initializing a likelihood calculator 
                            (not an MCMC run), we do *not* need to rename or
                            rescale for log-space. That code is commented out below.
                            The ranges are neglible and not used by the likelihood calculator.
                            They are just there as a placeholder, such that the MontePython DATA class object
                            can be initialized with the correct parameter names.
        ...
        print(
            f"MontePython input file created: {likelihood_calc_param_file}", flush=True
        )

    elif param.mcmc_sampler == "cobaya":
        # Initialize the Cobaya info dictionary
        info = {"likelihood": {}, "params": {}, "theory": {}}
        ...
        lkls = {
            "Planck_highl_TTTEEE_lite": {
                "name": "planck_2018_highl_plik.TTTEEE_lite",
                "clik": os.path.join(path_clik, "hi_l/plik_lite"),
            },
            "Planck_lowl_TT": {
                "name": "planck_2018_lowl.TT_clik",
                "clik": os.path.join(path_clik, "low_l/commander"),
            },
            "Planck_lowl_EE": {
                "name": "planck_2018_lowl.EE_clik",
        ...
        for par, interval in param.parameters.items():
            ...
            """
            Presumably this code is also redundant for likelihood initialization. But Cobaya likelihood-calculator is not implemented yet.
            if par in param.log_priors:
                if xmin != -1e+32:
                    xmin = np.log10(xmin)
                else:
            ...
            if par.startswith("ln10^{10}A_s"):
                ...
                info["params"]["100*theta_s"][
                    "value"
                ] = "lambda theta_s_100: theta_s_100"
