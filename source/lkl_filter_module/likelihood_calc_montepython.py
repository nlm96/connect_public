    ...
    def __new__(cls, param_connect, id):
        ...
        if not hasattr(cls, "mp"):
            ...
            with contextlib.redirect_stdout(
                open(os.path.join(output_folder, f"montepython/rank_{id}.out"), "w")
            ):
                with contextlib.redirect_stderr(
                    open(os.path.join(output_folder, f"montepython/rank_{id}.err"), "w")
                ):
                    print(
                        "Initializing MontePython on process for the first time.",
                        flush=True,
                        file=sys.stdout,
                    )
            ...
            print(
                "MontePython was already initialized, returning existing instance.",
                flush=True,
            )
