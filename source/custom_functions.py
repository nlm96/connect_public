...
class LossFunctions:
    ...
    def cosmic_variance(self, y_actual, y_pred):
        ...
        # If the loss is nan, return 1e20
        # loss = tf.where(
        #     tf.math.is_nan(loss), tf.constant(1e20, dtype=tf.float32), loss
        # )
        return loss
...
class ActivationFunctions:
    ...
    def get_config(self):
        return {
            ...
            # Variables like self.alpha are not serialized here,
            #
        }
    ...
    def nygaard(self, x):
        ...
        exp_term = tf.math.exp(-tf.math.multiply(self.beta, x))
        inverse_one_plus_exp = tf.math.divide(1, 1 + exp_term)
        mult_elementwise = tf.math.multiply(
            inverse_one_plus_exp, self.gamma - self.alpha
        )
