...
if rank == 0:
    try:
        print(
            f"\n\n[MASTER] Calculating CLASS models for iteration {iteration}\n",
            flush=True,
        )
    except:
        print(
            f"\n\n[MASTER] Calculating CLASS models for initial sampling \n", flush=True
        )
...
if param.memory_safe and rank > 0:
    ...
    child_comm = MPI.COMM_SELF.Spawn(
        sys.executable,
        args=[
            os.path.join(CONNECT_PATH, "source", "calc_models_mpi_memsafe_child.py"),
            "--child-mode",
            param_file,
            CONNECT_PATH,
            str(rank),
    ...
    def respawn_child():
        ...
        try:
            ...
            child_comm = MPI.COMM_SELF.Spawn(
                ...
                args=[
                    os.path.join(
                        CONNECT_PATH, "source", "calc_models_mpi_memsafe_child.py"
                    ),
                    "--child-mode",
                    param_file,
                    CONNECT_PATH,
...
## rank > 0 (slaves)
    ...
    def final_sync():
        ...
        for fname in file_paths:
            ...
            try:
                ...
                print(
                    f"{prefix} File {fname} does not exist. Skipping sync.",
                    file=sys.stderr,
                )
                pass
                ...
                print(
                    f"{prefix} Permission error for file {fname}. Skipping sync.",
                    file=sys.stderr,
                )
                pass
            except Exception as e:
                print(
                    f"{prefix} Error syncing file {fname}: {repr(e)}", file=sys.stderr
                )
                pass

        # Determine the number of samples in each file
        time.sleep(5)  # Wait for files to be synced
        ...
        def count_samples(fname):
            try:
                ...
                print(
                    f"{prefix} Error counting samples in {fname}: {repr(e)}",
                    file=sys.stderr,
                )
                return None
        ...
        if not sample_counts:
            print(
                "{prefix} No valid files to check sample counts. Skipping consistency check.",
                file=sys.stderr,
            )
            return
        ...
        def expected_lines(fname):
            try:
                ...
                print(
                    f"{prefix} Error computing expected lines for {fname}: {repr(e)}",
                    file=sys.stderr,
                )
                return None
        ...
        for fname in file_paths:
            ...
            if expected is None:
                print(
                    f"{prefix} couldn't determine expected lines for {fname}. Skipping truncation for all files.",
                    file=sys.stderr,
                )
                return
            try:
                ...
                print(
                    f"{prefix} Error reading file {fname} during truncation: {repr(e)}",
                    file=sys.stderr,
                )
                continue
            current = len(lines)
            if current > expected:
                print(
                    f"{prefix} [FINAL_SYNC] Truncating {fname} from {current} to {expected} lines",
                    file=sys.stderr,
                    flush=True,
                )
                try:
                    ...
                    print(
                        f"{prefix} Error truncating file {fname}: {repr(e)}",
                        file=sys.stderr,
                    )
            elif current < expected:
                print(
                    f"{prefix} [FINAL_SYNC] Warning: {fname} has only {current} lines but expected {expected} lines",
                    file=sys.stderr,
                )
    ...
    while True:
        ...
        if param.use_likelihood_filter and param.sampling == "iterative":
            ...
            if type(received_data).__name__ == "str":
                try:
                    time.sleep(5)  # Wait to for files to be synced
                    final_sync()  # Try syncing files in case they were not synced
                except Exception as e:
                    print(
        ...
        try:
            ...
            while True:
                ...
                if respawn_count >= max_respawn:
                    ...
                    comm.send("IDLE", dest=0, tag=500)  # Send idle message to master
                ...
                # CLASS computations must not take longer than 200 seconds
                signal.alarm(param.max_time_for_theory + 40)
                ...
                if result.get("success", False):
                    ...
                    if model is not None:
                        if param.use_likelihood_filter:
                            if loglkl_true is not None:
                                success = True
                            else:
                                success = False
                        else:
    ...
    if param.memory_safe:
        ...
        signal.alarm(param.max_time_for_theory + 45)  # 10 seconds timeout
...
TIMEOUT_SECONDS = max(600, param.max_time_for_theory + 60)
...
if rank > 0:
    ...
    while not comm.Iprobe(source=0, tag=1000):
        ...
        if time.time() - start_wait > TIMEOUT_SECONDS + 100:  # Failsafe extra timeout
            print(
...
# -------------------- Master Rank (0) --------------------
    ...
    while len(done_ranks) < N_slaves:
        ...
        if len(abort_ranks) > 0:
            ...
            time.sleep(10)  # wait to ensure buffer is flushed and written to all files
            MPI.COMM_WORLD.Abort(1)
