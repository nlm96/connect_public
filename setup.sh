...
then
	...
	make clean
	make -j
    class_path="${PWD}/resources/class_public"  # Set class_path here
	cd $connect_path
	echo "--> ...done!"
    fi
fi
...
# Set up likelihood_calc.conf
echo "--> Setting up likelihood calculator configuration file..."
likelihood_calc_conf_template="${PWD}/source/lkl_filter_module/likelihood_calc.conf.template"
likelihood_calc_conf="${PWD}/source/lkl_filter_module/likelihood_calc.conf"
cp "$likelihood_calc_conf_template" "$likelihood_calc_conf"

if [ ! -z "$class_path" ] # Add 'cosmo' path if 'class_path' is set
then
    echo "path['cosmo'] = '${class_path}'" >> "$likelihood_calc_conf"
else
    echo "Warning: 'class_path' is not set. 'path['cosmo']' will not be included."
fi

if [ ! -z "$clik_path" ] # Add 'clik' path if 'clik_path' is set
then
    echo "path['clik'] = '${clik_path}'" >> "$likelihood_calc_conf"
else
    echo "Warning: 'clik_path' is not set. 'path['clik']' will not be included."
fi

if [ $Ans2 == $YES ] # Handle 'montepython' path based on installations
then
    # MontePython is installed
    if [ ! -z "$montepython_path" ]
    then
        echo "path['montepython'] = '${montepython_path}'" >> "$likelihood_calc_conf"
    else
        echo "Warning: 'montepython_path' is not set. 'path['montepython']' will not be included."
    fi
elif [ $Ans3 == $YES ] # Cobaya is installed
then
    line3=$(grep -hr "montepython" "$likelihood_calc_conf") # Try to preserve existing 'path['montepython']' if it exists
    if [ ! -z "$line3" ]
    then
        echo "$line3" >> "$likelihood_calc_conf"
    else
        echo "Warning: 'path['montepython']' not found. It will not be included."
    fi
fi
