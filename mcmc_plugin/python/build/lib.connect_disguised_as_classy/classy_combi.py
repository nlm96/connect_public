...
class Class(real_classy.Class):
    ...
    def load_model(self, model_name=None):
        ...
        try:
            try:
                self.model = tf.keras.models.load_model(os.path.join(CONNECT_PATH,'trained_models',name+'low'), compile=False)
                self.model2 = tf.keras.models.load_model(os.path.join(CONNECT_PATH,'trained_models',name+'high'), compile=False)## hard coded
            except:
                self.model = tf.keras.models.load_model(name+'low', compile=False)
                self.model2 = tf.keras.models.load_model(name+'high', compile=False)## hard coded
        except:
    ...
    def set(self, *args, **kwargs):
        ...
        if bool(input_parameters):
            ...
            try:
                ...
                pass
            try:
                self.switch_name = input_parameters.pop('switch_name')
            except:
                pass
            try:
                self.connect_only = input_parameters.pop('connect_only')
            except:
                pass
            super(Class, self).set(input_parameters)
    ...
    def compute(self, level=None):
        ...
        try:
            params = []
            use_low = True ## hard coded
            for par_name in self.param_names:
                if par_name in self.pars:
                    params.append(self.pars[par_name])
                    if par_name == self.switch_name and self.pars[par_name] > 10**(-2.5): ## hardcoded
                        use_low = False ## hard coded
            ...
            if use_low: ## hard coded
                self.output_predict = self.model(v).numpy()[0]
            else: ## hard coded
                self.output_predict = self.model2(v).numpy()[0] ## hard coded
        except:
