...
class Analyze_likelihoods:
    ...
    def _plot_one_panel(
        ...
        # save databased x-limits before drawing threshold/best-fit lines
        # so they don't expand the axis range.

        data_xlim = None
        if self.x_range is None:  # kun hvis brugeren IKKE har sat en manuel x-range
...
def legacy_load_class_contours(class_plots_path, param1, param2, basename):
    """
    ...
    def get_name_variations(p):
        ...
        if p == "100*theta_s":
            ...
            # e.g., log10G_eff_ncdm_interacting
            variations.append(p.replace("log10", "log10^")) # for triangleplot.py compatibility
        else:
    ...
    # Try all combinations to find the file
    ...
    try:
        # This file format is tricky. Let's re-parse based on headers.
        x95_list, y95_list = [], []
        x68_list, y68_list = [], []
        ...
        with open(dat_path) as f:
            ...
            for line in f:
                if line.strip().startswith('# contour for confidence level 0.95'):
                    if current_x: # Save previous segment
                        current_list_x.append(current_x)
                        current_list_y.append(current_y)
                    current_list_x, current_list_y = x95_list, y95_list
                    current_x, current_y = [], []
                    continue
                    ...
                    if current_x: # Save previous segment
                        current_list_x.append(current_x)
                        current_list_y.append(current_y)
                    current_list_x, current_list_y = x68_list, y68_list
                    current_x, current_y = [], []
                    continue
                ...
                if not line.strip() and current_list_x is not None:
                    # Empty line signifies a new path segment
                    if current_x:
                        current_list_x.append(current_x)
                        current_list_y.append(current_y)
                    current_x, current_y = [], []
                ...
                if current_list_x is None:
                    continue # We haven't hit a header yet
        ...
        # START OF NEW RESCALING BLOCK
        # -----------------------------------------------------------------
        # This fixes the 100*omega_b scaling mismatch between
        # CONNECT (which uses physical omega_b) and MontePython MCMC
        # (which samples 100*omega_b).

        # param1 corresponds to X-axis data (x95_list, x68_list)
        if param1 == 'omega_b':
            # Safety check: only rescale if data looks scaled (e.g., > 1.0)
            if x95_list and x95_list[0] and np.mean(x95_list[0]) > 1.0:
                x95_list = [[x / 100.0 for x in sublist] for sublist in x95_list]
                x68_list = [[x / 100.0 for x in sublist] for sublist in x68_list]

        # param2 corresponds to Y-axis data (y95_list, y68_list)
        if param2 == 'omega_b':
            # Safety check: only rescale if data looks scaled
            if y95_list and y95_list[0] and np.mean(y95_list[0]) > 1.0:
                y95_list = [[y / 100.0 for y in sublist] for sublist in y95_list]
                y68_list = [[y / 100.0 for y in sublist] for sublist in y68_list]
        
        # -----------------------------------------------------------------
        # END OF NEW RESCALING BLOCK

        # if switch:
        #     return y95_list, x95_list, y68_list, x68_list
        # else:
        #     return x95_list, y95_list, x68_list, y68_list
        
        if switch:
            # We found the Y-X file (e.g., H0-omega_b.dat).
            # The content is (X_DATA, Y_DATA) (e.g., omega_b, H0).
            # x95_list has X_DATA, y95_list has Y_DATA.
            # We must return (X_DATA, Y_DATA, ...).
            return x95_list, y95_list, x68_list, y68_list
        else:
            # We found the X-Y file (e.g., omega_b-H0.dat).
            # This is the "swapped" case. We assume the file
            # content is also (X_DATA, Y_DATA) but saved with
            # the X-Y name, OR that the content is (Y_DATA, X_DATA).
            # Based on the original triangleplot.py, the lists
            # should be swapped.
            return y95_list, x95_list, y68_list, x68_list
