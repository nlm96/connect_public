...
# Loop over the three spectra (TT, TE, EE)
for k, output in enumerate(output_info['output_Cl']):
    ...
    # Predictions and true values for the power spectra
    Cl_predict = out_predict[output]
    Cl_data = out_data[output]
    ...
    # Upper row: Plot CMB power spectra
    if pickle_file and normalize == 'factor':
        axs[0, k].plot(ll, Cl_dat_sp(ll)/normalize_factor, 'k-', lw=3, label='CLASS')  # CLASS spectra
    else:
        axs[0, k].plot(ll, Cl_dat_sp(ll), 'k-', lw=3, label='CLASS')  # CLASS spectra
    
    if pickle_file and normalize == 'factor':
        axs[0, k].plot(ll, Cl_pre_sp(ll)/normalize_factor, 'r--', lw=3, label='CONNECT')  # CONNECT spectra
    else:
        axs[0, k].plot(ll, Cl_pre_sp(ll), 'r--', lw=3, label='CONNECT')  # CONNECT spectra
    
    axs[0,k].axvline(x=change, linestyle="-", color="lightgrey", zorder=-3)

    axs[0,k].set_xscale('planck')
    axs[0,k].set_xlim([2,2500])
    axs[0, k].set_title(f'{output.upper()}')
    axs[0, k].set_xlabel(r'$\ell$')
    axs[0, k].set_ylabel(r'$C_{\ell} \times \ell(\ell+1)/2\pi$')

    # Lower row: Plot errors/residuals using the existing working format
    y_max = 0
    y_min = 0
    for i, path in enumerate(model_paths): 
        model_name = model_names[i]
...
if not os.path.isdir(model_paths[0]+'/plots'):
    os.mkdir(model_paths[0]+'/plots')
# Save the combined multipanel plot (CMB spectra + errors)
